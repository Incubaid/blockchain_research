...
struct User {
  id @0 :UInt32; #4 bytes, means max 4billion entries max 
  epoch @1: UInt32; #moddate
  pubkey @2; Data; #32 bytes (use https://github.com/philanc/luatweetnacl)
  history @3: UInt32; #if there is a previous version of this user (e.g. if pubkey changes)
  sign @4 :Data; #48 bytes: has been signed by secret key of User & returns the 16 byte blake hash 
                        # of previousobj:sign + current:region+rid+epoch+pubkey+history+link
  link @5 :UInt32; #link to Base object which has all data for this user in namespace 1 which is special namespace for user

}
#size: 4+4+32+4+48 = 92 bytes
#2 billion entries = <175 GB

  
struct Base {
  id @1 :UInt32;        #4 bytes, means max 4billion entries max per namespace
  ...
  data  @5 :Data;       #the payload, if a new version is made with empty data, this means this object has been deleted, first byte = position in ring
  crc  @6: UInt32;
  namespace @7: UInt32;
  admingroups @8 :List(UInt32);  #list of authors who can modify, only if author is member of admins list the it can create a new version of this obj
